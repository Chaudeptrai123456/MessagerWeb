...
# Import từ vector_store (Qdrant)
from vector_store import list_all_collections_data, add_document, query_similar
...
from fred_ingest import ingest_default_series, query_macro_context
...
# Đăng ký middleware (nếu muốn bật auth)
...
@app.post("/add")
async def add_to_vector_db(user_id: str = Form(...), content: str = Form(...)):
    add_document(user_id, doc_id=str(uuid.uuid4()), text=content, topic="manual")
    return {"message": f"Đã thêm văn bản cho user {user_id}"}
...
async def upload_pdf(user_id: str = Form(...), file: UploadFile = File(...)):
    ...
    add_document(user_id, doc_id=file_id, text=text, topic="pdf")

    return {
        "message": f"Đã lưu nội dung từ PDF cho user {user_id}",
...
@app.get("/vector_dump")
async def vector_dump():
    results = list_all_collections_data()
    return results
